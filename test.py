...
parser.add_argument('--output_dir', default='./predict_FSA-Net_noFSAS', type=str, help='output dir')
...
if __name__ == "__main__":
    ...
    ckpt = torch.load(snapshot, weights_only=True)
    # dict = {'swin_unet.' + key: value for key, value in ckpt['model'].items()}
    ckpt = {k[10:]:v for k, v in ckpt.items()}
    msg = net.swin_unet.load_state_dict(ckpt)
    print("self trained swin unet", msg)
    snapshot_name = snapshot.split('/')[-1]

    log_folder = './test_log/test_log_FSA-Net+Mutil'
