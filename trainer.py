...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in tqdm(enumerate(train_loader), desc=f"Train: {epoch_num}", total=len(train_loader),
            ...
            # min_val = torch.min(image_batch.view(batch_size, -1), dim=1)[0].min()
            # max_val = torch.max(image_batch.view(batch_size, -1), dim=1)[0].max()
            # image_batch = (image_batch - min_val) * 8 / (max_val - min_val)
        ...
        batch_loss = 0.2 * batch_ce_loss + 0.8 * batch_dice_loss
        ...
        if (epoch_num + 1) % args.eval_interval == 0:
            ...
            batch_ce_loss = 0
            with torch.no_grad():
                for i_batch, sampled_batch in tqdm(enumerate(val_loader), desc=f"Val: {epoch_num}", total=len(val_loader),
                                           leave=False):
                    image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
                    # min_val = torch.min(image_batch.view(batch_size, -1), dim=1)[0].min()
                    # max_val = torch.max(image_batch.view(batch_size, -1), dim=1)[0].max()
